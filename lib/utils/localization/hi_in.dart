// lib/utils/localization/hi_in.dart

const Map<String, String> hiIN = {
  // Settings Page
  'settings.title': 'सेटिंग्स',
  'settings.language': 'भाषा',
  'settings.selectLanguage': 'भाषा चुनें',
  'settings.profile': 'प्रोफ़ाइल',
  'settings.currency': 'मुद्रा',
  'settings.selectCurrency': 'मुद्रा चुनें',

  // Currency Names
  'currency.try': 'तुर्की लीरा (₺)',
  'currency.usd': 'अमेरिकी डॉलर (\$)',
  'currency.eur': 'यूरो (€)',
  'currency.gbp': 'ब्रिटिश पाउंड (£)',
  'currency.jpy': 'जापानी येन (¥)',
  'currency.rub': 'रूसी रूबल (₽)',
  'currency.inr': 'भारतीय रुपया (₹)',
  'currency.cny': 'चीनी युआन (¥)',
  'currency.krw': 'कोरियाई वॉन (₩)',
  'currency.myr': 'मलेशियाई रिंगित (RM)',
  'currency.brl': 'ब्राज़ीलियाई रियल (R\$)',
  'currency.zar': 'दक्षिण अफ्रीकी रैंड (R)',
  'currency.sek': 'स्वीडिश क्रोना (kr)',
  'currency.chf': 'स्विस फ्रैंक (CHF)',
  'currency.aud': 'ऑस्ट्रेलियाई डॉलर (A\$)',
  'currency.cad': 'कनाडाई डॉलर (C\$)',

  // Profile Page
  'personal_info': 'व्यक्तिगत जानकारी',
  'preferences': 'प्राथमिकताएँ',
  'about_app': 'ऐप के बारे में',
  'app_version': 'ऐप संस्करण',
  'privacy_policy': 'गोपनीयता नीति',
  'privacy_policy_content': """<html>
<head>
<meta charset="UTF-8">
<title>गोपनीयता नीति</title>
</head>
<body>
<h1>PocketPath गोपनीयता नीति</h1>
<p>अंतिम अद्यतन: 15/05/2025</p>

<h2>परिचय</h2>
<p>हम PocketPath एप्लिकेशन का उपयोग करते समय आपकी गोपनीयता का सम्मान करते हैं। यह गोपनीयता नीति बताती है कि PocketPath आपकी व्यक्तिगत जानकारी को कैसे एकत्र करता है, उपयोग करता है और सुरक्षित रखता है।</p>

<h2>हम जो जानकारी एकत्र करते हैं</h2>
<ul>
  <li><strong>खाता जानकारी:</strong> आपका ईमेल पता, पहला नाम और उपनाम।</li>
  <li><strong>वित्तीय डेटा:</strong> आपके लक्ष्यों, खर्चों और आय के बारे में आपके द्वारा दर्ज की गई जानकारी।</li>
  <li><strong>उपयोग डेटा:</strong> आपके द्वारा एप्लिकेशन का उपयोग करने के तरीके के बारे में डेटा।</li>
</ul>

<h2>डेटा का उपयोग</h2>
<p>हम एकत्रित जानकारी का उपयोग निम्नलिखित उद्देश्यों के लिए करते हैं:</p>
<ul>
  <li>हमारी सेवा प्रदान करना और सुधारना</li>
  <li>आपके खाते का प्रबंधन करना</li>
  <li>व्यक्तिगत अनुभव प्रदान करना</li>
  <li>एप्लिकेशन की कार्यक्षमता बढ़ाना</li>
</ul>

<h2>डेटा सुरक्षा</h2>
<p>हम आपके व्यक्तिगत डेटा की सुरक्षा के लिए उद्योग-मानक सुरक्षा उपाय लागू करते हैं। आपकी जानकारी Firebase इंफ्रास्ट्रक्चर में सुरक्षित रूप से संग्रहीत है।</p>

<h2>डेटा शेयरिंग</h2>
<p>हम आपकी स्पष्ट सहमति के बिना तीसरे पक्ष के साथ आपकी व्यक्तिगत जानकारी साझा नहीं करते हैं। हम केवल तभी जानकारी साझा कर सकते हैं जब हमारे कानूनी दायित्वों को पूरा करने के लिए आवश्यक हो।</p>

<h2>आपके अधिकार</h2>
<p>आपके व्यक्तिगत डेटा के संबंध में आपके पास निम्नलिखित अधिकार हैं:</p>
<ul>
  <li>अपने डेटा तक पहुंच</li>
  <li>अपने डेटा का सुधार</li>
  <li>अपने डेटा को हटाने का अनुरोध</li>
  <li>अपने डेटा के उपयोग को प्रतिबंधित करना</li>
</ul>

<h2>संपर्क</h2>
<p>यदि आपके पास हमारी गोपनीयता नीति के बारे में कोई प्रश्न या चिंता है, तो कृपया हमसे संपर्क करें।</p>
</body>
</html>""",
  'developer': 'डेवलपर: फ़ुरकान अकीफ़ इस्लेक',
  'noEmail': 'कोई ईमेल नहीं मिला',
  'version.number': '1.0.0',
  'profile.deleteAccountImportant': 'खाता हटाएं',
  'profile.deleteAccountImportantText':
      'क्या आप वाकई अपना खाता हटाना चाहते हैं? यह क्रिया पूर्ववत नहीं की जा सकती।',
  'profile.cancel': 'रद्द करें',
  'profile.delete': 'हटाएं',
  'profile.errorDeletingAccount': 'खाता हटाने में त्रुटि',
  'profile.deleteAccountButton': 'खाता हटाएं',

  // Profile Page
  'logout': 'लॉगआउट',
  'account': 'खाता',
  "general.done": "पूरा हो गया",

  // Login Page
  "login.title": "अपने खाते में लॉगिन करें",
  "login.noAccount": "खाता नहीं है?",
  "login.signUp": "साइन अप करें",
  "login.emailLabel": "आपका ईमेल",
  "login.passwordLabel": "अपना पासवर्ड दर्ज करें",
  "login.loginButton": "लॉगिन",
  "login.forgotPassword": "पासवर्ड भूल गए?",
  "login.resetPasswordSent": "पासवर्ड रीसेट ईमेल भेज दिया गया!",
  "login.pleaseEnterEmail": "कृपया अपना ईमेल पता दर्ज करें।",

  // Login Error Messages
  "login.error.invalidEmail": "आपने एक अमान्य ईमेल पता दर्ज किया है।",
  "login.error.userDisabled": "उपयोगकर्ता को निष्क्रिय कर दिया गया है।",
  "login.error.wrongCredentials":
      "दर्ज की गई जानकारी गलत है। कृपया पुन: प्रयास करें।",
  "login.error.emailInUse": "यह ईमेल पता पहले से उपयोग में है।",
  "login.error.notAllowed": "इस प्रक्रिया की अनुमति नहीं थी।",
  "login.error.verifyEmail":
      "लॉगिन करने से पहले कृपया अपने ईमेल पते की पुष्टि करें।",
  "login.error.generic": "एक त्रुटि हुई है। कृपया बाद में पुन: प्रयास करें।",

  // Register Page
  "register.title": "PocketPath ऐप में पंजीकरण करें",
  "register.haveAccount": "पहले से ही खाता है?",
  "register.signIn": "साइन इन करें",
  "register.fullName": "पूरा नाम",
  "register.surname": "उपनाम",
  "register.email": "आपका ईमेल",
  "register.password": "अपना पासवर्ड दर्ज करें",
  "register.confirmPassword": "अपना पासवर्ड फिर से दर्ज करें",
  "register.registerButton": "पंजीकरण करें",

  // Register Error Messages
  "register.error.passwordMismatch": "पासवर्ड मेल नहीं खाते।",
  "register.error.fillAllFields": "सभी फ़ील्ड भरें।",
  "register.error.generic": "एक त्रुटि हुई",

  // Register Confirm Page
  "register_confirm.title": "अपना ईमेल जांचें",
  "register_confirm.message":
      "पंजीकरण सफल रहा। कृपया अपने ईमेल पते पर भेजे गए लिंक से अपने खाते की पुष्टि करें।",
  "register_confirm.button": "पुष्टि करें",

  // Password Confirm Page
  "password_confirm.title": "अपना ईमेल जांचें",
  "password_confirm.message":
      "हमने आपको अपना पासवर्ड रीसेट करने के लिए जानकारी भेज दी है",
  "password_confirm.button": "पुष्टि करें",

  // Reset Password Page
  "reset_password.title": "पासवर्ड रीसेट करें",
  "reset_password.message":
      "अपना ईमेल पता दर्ज करें और हम आपको पासवर्ड रीसेट करने के तरीके के बारे में आगे के निर्देश भेजेंगे।",
  "reset_password.emailLabel": "आपका ईमेल",
  "reset_password.button": "रीसेट करें",
  "reset_password.error.enterEmail": "कृपया अपना ईमेल पता दर्ज करें।",

  // Introduction Pages
  "intro.skip": "छोड़ें",
  "intro.next": "अगला",
  "intro.done": "पूर्ण",

  // Introduction Page 1
  "intro.page1.text":
      "क्या आपको अपने खर्च और बचत को नियंत्रित करना मुश्किल लगता है? पॉकेटपाथ के साथ, यह समस्या आसानी से हल हो जाती है।",

  // Introduction Page 2
  "intro.page2.title": "पहला कदम",
  "intro.page2.text":
      "आपको खर्च नियंत्रण के लिए एक लक्ष्य निर्धारित करने की आवश्यकता है। एक बार जब आपके पास लक्ष्य हो, तो आप अपने खर्च को रिकॉर्ड कर सकते हैं।",
  "intro.page2.quote":
      "यदि आप लक्ष्य निर्धारित करते हैं और उन्हें पूरे दृढ़ संकल्प के साथ पीछा करते हैं, तो आपकी प्रतिभाएं आपको ऐसे स्थानों पर ले जाएंगी जो आपको आश्चर्यचकित करेंगे।",
  "intro.page2.author": "- लेस ब्राउन",

  // Introduction Page 3
  "intro.page3.title": "दूसरा कदम",
  "intro.page3.text":
      "आप लक्ष्यों को देख सकते हैं और तदनुसार खर्च और आय निर्धारित कर सकते हैं। आप अपने नियमित खर्चों और अपने पूरे किए गए लक्ष्यों का ट्रैक रख सकते हैं।",
  "intro.page3.quote":
      "उस नाविक के लिए कोई अनुकूल हवा नहीं है जो नहीं जानता कि वह कहां जा रहा है।",
  "intro.page3.author": "- सेनेका",

  // Home Page Components
  "home.title.hello": "नमस्ते",
  "home.title.welcomeBack": "वापसी पर स्वागत है!",
  "home.budget.balance": "शेष राशि",
  "home.budget.noTarget": "आपने अभी तक कोई लक्ष्य नहीं दर्ज किया है",
  "home.budget.createNewTask": "नया कार्य बनाएं",
  "home.budget.selectTargetPlan": "लक्ष्य योजना चुनें",
  "home.budget.budget": "बजट:",

  // Tab Period
  "home.tab.day": "दिन",
  "home.tab.month": "माह",
  "home.tab.yearly": "वार्षिक",

  // Cost Component
  "home.cost.expense": "व्यय",
  "home.cost.income": "आय",
  "home.cost.delete.title": "लागत हटाएं",
  "home.cost.delete.message": "क्या आप वाकई इस लागत को हटाना चाहते हैं?",
  "home.cost.delete.cancel": "रद्द करें",
  "home.cost.delete.confirm": "हटाएं",

  // Categories
  "category.shopping": "खरीदारी",
  "category.healthy": "स्वास्थ्य",
  "category.food": "भोजन",
  "category.school": "स्कूल",
  "category.holiday": "छुट्टी",
  "category.electronic": "इलेक्ट्रॉनिक",
  "category.other": "अन्य",

  // Add Cost Page
  "addcost.title": "लागत जोड़ें",
  "addcost.save": "सहेजें",
  "addcost.recorded": "रिकॉर्ड किया गया",
  "addcost.success": "लागत सफलतापूर्वक रिकॉर्ड की गई!",
  "addcost.error": "त्रुटि",
  "addcost.invalid_budget": "अमान्य बजट राशि",
  "addcost.fill_fields": "कृपया सभी फ़ील्ड भरें",
  "addcost.select_target": "लक्ष्य चुनें",
  "addcost.select_target_hint": "\"स्टॉक वॉलेट\" जैसे सक्रिय लक्ष्य चुनें",
  "addcost.cost_type": "लागत प्रकार",
  "addcost.type_name_hint": "\"NASDAQ ETF\" जैसा नाम टाइप करें",
  "addcost.budget_amount": "बजट राशि",
  "addcost.category": "श्रेणी",
  "addcost.expense": "व्यय",
  "addcost.income": "आय",
  "addcost.select_a_target": "एक लक्ष्य चुनें",

  // Analysis Page
  "analysis.title": "विश्लेषण",
  "analysis.select_target": "लक्ष्य चुनें",
  "analysis.please_select_target": "कृपया लक्ष्य चुनें",
  "analysis.expenditures": "व्यय",
  "analysis.savings": "बचत",
  "analysis.total_expenditures": "कुल व्यय",
  "analysis.total_savings": "कुल बचत",
  "analysis.target_progress": "लक्ष्य प्रगति",
  "analysis.available": "उपलब्ध",
  "analysis.target": "लक्ष्य",

  // Menu Page
  "menu.home": "होम",
  "menu.analysis": "विश्लेषण",
  "menu.targets": "लक्ष्य",
  "menu.add_target": "लक्ष्य जोड़ें",
  "menu.profile": "प्रोफ़ाइल",

  // Target Pages
  "target.title": "लक्ष्य",
  "target.balance": "बैलेंस",
  "target.target_type": "लक्ष्य प्रकार",
  "target.type_name": "प्रकार का नाम",
  "target.date_picker": "तिथि चयनकर्ता",
  "target.select_date": "तिथि चुनें",
  "target.budget_amount": "बजट राशि",
  "target.save": "सहेजें",
  "target.recorded": "रिकॉर्ड किया गया",
  "target.success": "लक्ष्य सफलतापूर्वक रिकॉर्ड किया गया!",
  "target.error": "त्रुटि",
  "target.invalid_budget": "अमान्य बजट राशि",
  "target.fill_fields": "कृपया सभी फ़ील्ड भरें",
  "target.set_new_target": "नया लक्ष्य निर्धारित करें",
  "target.passive": "निष्क्रिय",
  "target.active": "सक्रिय",
  "target.all": "सभी"
};
